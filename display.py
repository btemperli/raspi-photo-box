...
class Display():
    ...
    def __init__(self):
        ...
        self.label.pack(padx=200, pady=100)

        self.show_video = tk.BooleanVar(value=True)
        ttk.Button(glv.root_window, text="Photo aufnehmen!", command=self.start_countdown).pack()
    ...
    def update_countdown(self, count):
        if count > 0:
            ...
            self.label.config(height=(glv.window_height / 2))
            ...
            self.label.config(height=glv.window_height)
